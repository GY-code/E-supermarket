        ...
        <div v-if="cartList.length">
          <div class="ui-cart">
            <div>
                ...
                <div class="cart-group divide pr" :data-productid="item.productId">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <!--删除按钮-->
                      <div class="operation">
                        <a class="items-delete-btn" @click="cartdel(Number(item.productId))"></a>
                      </div>
                      <!--商品数量-->
                      <div>
                        <!--总价格-->
              ...
              <!--标题-->
              <div class="cart-table-title">
                <span class="name">优惠券信息</span> <span class="operation">失效日期</span> <span
                class="subtotal">减额</span> <span class="num">满额</span> <span class="price1">类别</span>
              </div>
              <!--列表-->
              <div class="cart-table" v-for="(item0,i0) in couponList" :key="i0">
                <div class="cart-group divide pr" :data-productid="item0.couponId">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      <!--勾选-->
                      <div class="items-choose">
<!--                        <el-checkbox v-model="item.checked"></el-checkbox>-->
<!--                        <span id="fuckdb" class="blue-checkbox-new "-->
                        <span class="blue-checkbox-new " :class="{'checkbox-on':item0.Fchecked === 1}"
                              @click="editCart('cccheck', item0)"></span>
<!--                        <el-checkbox v-model="pig" @change="editCart('cccheck,')"></el-checkbox>-->
                      </div>
                      <!--图片-->
                      <div class="items-thumb fl">
                        <img :src="item0.url">
                      ...
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px"> {{item0.bonus}}</div>
                        <!--数量-->
<!--                        <div>-->
<!--                          {{item.criterion}}-->
<!--                        </div>-->
                        ...
                        <!--价格-->
                        <div class="price1"> {{item0.name}}</div>
                        <div class="items-thumb fl" style="padding-top: 27px;font-size: 20px" v-if="item0.rec">
                          &#128525;
                        </div>
                      </div>
                    </div>
...
<!--                <y-button :classStyle="checkNum > 0 && submit?'main-btn':'disabled-btn'"-->
                <y-button :classStyle="checkNum > 0 && 'main-btn'"
                          class="big-main-btn"
...
<script>
  // import $ from 'jquery'
  import { getPerfectCouponList, getCartList, cartEdit, editCheckAll, cartDel, delCartChecked } from '/api/goods'
  ...
  // import coupon from "../User/children/coupon";
  export default {
    data () {
      return {
        pig: 1,
    ...
    computed: {
      ...
      checkPrice () {
        ...
        this.couponList && this.couponList.forEach(ii => {
          if (ii.Fchecked === 1) {
            totalPrice -= (ii.bonus)
          }
        })
    ...
    methods: {
      ...
      editCheckAll () {
        ...
        editCheckAll({cartId: Number(this.userId), checked: Number(checkAll)}).then(res => {
          this.EDIT_CART({checked: checkAll})
        })
      },
      // 修改购物车
      _cartEdit (userId, productId, productNum, checked) {
        cartEdit({cartId: userId, productId, productNum, checked}).then(res => {
          if (res.success === true) {
            this.EDIT_CART({productId, checked, productNum})
          }
          this._getPerfectCouponList()
        })
      },
      ...
      editCart (type, item) {
        if (type === 'cccheck') {
          let c = (item.Fchecked === 1) ? 0 : 1
          this.$set(item, 'Fchecked', c)
          this.couponList.forEach(ii => {
            if (ii.name === item.name && ii.couponId !== item.couponId) {
              ii.Fchecked = 0
            }
          })
          this.$forceUpdate()
        ...
        console.log('zyg2' + item.checked)
        console.log('mm ' + item.checked)
        if (type && item) {
          ...
          if (type === 'check') {
            ...
            this._cartEdit(Number(this.userId), Number(productId), Number(productNum), Number(newChecked))
          }
        } else {
          console.log('缺少所需参数')
        }
      ...
      EditNum (productNum, productId, checked) { // 数量
        this._cartEdit(Number(this.userId), Number(productId), Number(productNum), Number(checked))
      },
      // 删除整条购物车
      cartdel (productId) {
        cartDel({cartId: Number(this.userId), productId}).then(res => {
      ...
      delChecked () {
        //
        getCartList({cartId: Number(getStore('userId'))}).then(res => {
          if (res.success === true) {
            res.result.forEach(item => {
              console.log('helloMe:' + item)
              if (item.checked === 1) {
                let productId = item.productId
                this.EDIT_CART({productId})
              }
        ...
        // 在表中删除选中的商品
        delCartChecked({cartId: Number(this.userId)}).then(res => {
          if (res.success !== true) {
            this.message('删除失败')
          }
        })
      },
      _getPerfectCouponList () {
        getPerfectCouponList({userId: Number(this.userId)}).then(res => {
          this.couponList = res.result
          this.couponList.forEach((item) => {
            let date = new Date(item.toTime)
            let year = date.getFullYear()
            let month = date.getMonth() + 1
            let day = date.getDate()
            month = month < 10 ? '0' + month : month
            day = day < 10 ? '0' + day : day
            item.toTime = year + '-' + month + '-' + day
          })
        })
      }
    },
    ...
    mounted () {
      ...
      this._getPerfectCouponList()
      this.INIT_BUYCART()
      console.log('hmbb' + this.cartList)
    },
...
<style lang="scss" rel="stylesheet/scss" scoped>
  ...
  .page-cart {
    ...
    .blue-checkbox-new, .blue-checkbox-new.checkbox-disable, .blue-checkbox-new.checkbox-on1 {
      display: inline-block;
      position: relative;
      width: 20px;
      height: 20px;
