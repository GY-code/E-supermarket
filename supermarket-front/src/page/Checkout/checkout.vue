        ...
        <div slot="content">
          <div class="box-inner ui-cart">
            <div>
              ...
              <!--列表-->
              <div class="cart-table" v-for="(item,i) in couponList" :key="i" v-if="item.Fchecked === 1">
                <div class="cart-group divide pr" :data-productid="item.couponId">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      <!--图片-->
                      <div class="items-thumb fl">
                      ...
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px"> {{item.toTime}}</div>
                        <!--数量-->
                        <div class="item-cols-num">
                          <span v-text="item.bonus"></span>
                        </div>
                        <!--价格-->
                        <div class="price"> {{item.criterion}}</div>
                        <!--价格-->
...
<script>
  import { getCartList, addressList, addressUpdate, addressAdd, addressDel, productDet, submitOrder, delCartChecked, insertCouponToOrder } from '/api/goods'
  ...
  export default {
    ...
    computed: {
      ...
      checkPrice () {
        ...
        this.couponList && this.couponList.forEach(item => {
          if (item.Fchecked === 1) {
            totalPrice -= (item.bonus)
          }
        })
    ...
    methods: {
      ...
      _getCartList () {
        getCartList({cartId: Number(this.userId)}).then(res => {
      ...
      _submitOrder () {
        ...
        submitOrder(params).then(res => {
          this.c_orderId = res.result
          console.log('aab' + this.c_orderId)
          console.log('aac' + res.result)
          if (res.success === true) {
          ...
          this.couponList.forEach(item => {
            if (item.checked === 1) {
              cidList.push(item.couponId)
            }
          })
          insertCouponToOrder({orderId: this.c_orderId, couponIdList: cidList}).then(res => {})
        })
      },
      // 付款
      payment (orderId) {
    ...
    created () {
      ...
      this.couponList = query.couponList
      console.error(this.cartList)
      console.error(this.couponList)
      this._addressList()
    },
    components: {
      YShelf,
