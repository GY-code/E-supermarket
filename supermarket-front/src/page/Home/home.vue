...
<!--      <el-button type="text" @click="dialogVisible1 = true">点击打开 Dialog</el-button>-->
      <el-dialog
        ...
        <span id="duihuan">确认兑换吗？将消耗{{couponPointValue}}积分</span>
        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogVisible1 = false">取 消</el-button>
          <el-button type="primary" @click="confirmRedemption">确 定</el-button>
        </span>
      </el-dialog>
    </div>
    ...
    <div v-for="(item,i) in home" :key="i">
      ...
      <div class="activity-panel" v-if="item.type === 1">
        <ul class="box">
          <li class="content" v-for="(iitem,j) in item.panelContents" :key="j" @click="linkTo(iitem)">
            <img class="i" :src="iitem.picUrl" style="width: 240px; margin-left: 40px">
      ...
      <section class="w mt30 clearfix" v-if="item.type === 3">
        ...
        <y-shelf ref="YShelf" :title="item.name" :isSec="item.id===11">
...
<script>
  ...
  import {getSecKillInitTime, getCouponPoint, redemptCoupon} from '../../api/goods'
  export default {
    data () {
      return {
        dialogMsg: '',
    ...
    methods: {
      confirmRedemption () {
        ...
        redemptCoupon({userId: Number(getStore('userId')), couponId: Number(this.c_couponId), point: Number(this.couponPointValue)}).then(res => {
      ...
      linkTo (item) {
        ...
        getCouponPoint({couponId: item.productId}).then(res => {
          this.c_couponId = item.productId
          this.couponPointValue = res.result
        })
      ...
      _getSecKillInitTime () {
        getSecKillInitTime().then(res => {
          this.$refs.YShelf.forEach(item => {
            item.hh = res.result.hh
            item.mm = res.result.mm
            item.ss = res.result.ss
          })
        })
      }
    },
    ...
    created () {
      this.play()
      this._getSecKillInitTime()
      setInterval(this.decreaseChildTime, 1000)
    },
    components: {
      YShelf,
      product,
