...
//        }
    ...
    @RequestMapping(value = "/member/payOrderSuc", method = RequestMethod.POST)
    @ApiOperation(value = "支付订单")
    public Result<Object> payOrderSuc(@RequestBody Order order) {
        System.out.println("orderId:"+order.getOrderId());
        String result = orderService.payOrderSuc(String.valueOf(order.getOrderId()));
        return new ResultUtil<Object>().setData(result);
    }

    @RequestMapping(value = "/member/commentOrder", method = RequestMethod.POST)
    @ApiOperation(value = "对订单的评论（也就是对骑手的评论）")
    public Result<Object> cancelOrder(@RequestBody DeliverOrder order) {
        int result = orderService.commentOrder(order.getOrderId(), order.getBuyerMessage());
        return new ResultUtil<Object>().setData(result);
    }
}
