...
public class CartController {
    ...
    public Result<Object> addCart(@RequestBody Cart cart){
        int result = cartService.addCart(cart.getCartId(),cart.getProductId(),1);
        return new ResultUtil<Object>().setData(result);
    }

    @RequestMapping(value = "/member/cartList",method = RequestMethod.POST)
    @ApiOperation(value = "获取购物车商品列表")
    public Result<List<Cart>> getCartList(@RequestBody Cart cart){
        List<Cart> list=cartService.getCartList(cart.getCartId());
        return new ResultUtil<List<Cart>>().setData(list);
    }
    ...
    public Result<Object> updateCartNum(@RequestBody Cart cart){
        ...
        int result=cartService.updateCartNum(cart.getCartId(),cart.getProductId(),cart.getProductNum(),cart.getChecked());
        return new ResultUtil<Object>().setData(result);
    }
    ...
    public Result<Object> deleteCartItem(@RequestBody Cart cart){
        ...
        int result=cartService.deleteCartItem(cart.getCartId(),cart.getProductId());
        return new ResultUtil<Object>().setData(result);
    }
